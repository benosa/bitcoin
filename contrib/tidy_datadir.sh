...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Thalercoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Thalercoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Thalercoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Thalercoin 0.7 datadir."
    ;;
  3)
    echo "Detected Thalercoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Thalercoin 0.8 datadir."
    ;;
esac
