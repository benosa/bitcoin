...
THALERCOIND=${1}/thalercoind
CLI=${1}/thalercoin-cli
...
B4ARGS="-datadir=$D4"
$THALERCOIND $THALERCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$THALERCOIND $THALERCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} tlr"
...
function StartThree {
  $THALERCOIND $THALERCOINDARGS $B1ARGS &
  B1PID=$!
  $THALERCOIND $THALERCOINDARGS $B2ARGS &
  B2PID=$!
  $THALERCOIND $THALERCOINDARGS $B3ARGS &
  B3PID=$!
}
